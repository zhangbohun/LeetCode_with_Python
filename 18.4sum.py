# -*- coding: utf-8 -*-
"""
Created on Fri Jul 10 12:30:52 2015
...
class Solution:
    ...
    def fourSum(self, nums, target):
        nums.sort()
        if len(nums) <4: # deal with special input
            return []
        elif len(nums) == 4:
            if sum(nums) == target:
                return [nums]
        ans = []
        for l in xrange(len(nums) -3):
            if nums[l] > target/4.0:
                break
            if nums[l] == nums[l-1]:
                continue
            for i in xrange(l+1,len(nums) -2):
                if nums[i] > (target-nums[l])/3.0:
                    break
                if nums[i] == nums[i-1]:
                    continue
                j = i+1
                if nums[j] > (target-nums[l]-nums[i])/2.0:
                            break
                k = len(nums) -1 # hence i < j < k
                while j<k: # if not cross line
                    temp_sum = nums[i] + nums[j] + nums[k]
                    if temp_sum == target-nums[l]:
                        ans.append((nums[l],nums[i], nums[j], nums[k]))
                    if temp_sum > target-nums[l]: # which means we need smaller sum, move k backward, remember we sort the array
                        k -= 1
                    else:
                        j += 1
